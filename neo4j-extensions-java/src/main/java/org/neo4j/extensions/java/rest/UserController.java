...
public class UserController implements UserClient {
    ...
    public Response create(Boolean indexingOn, Integer count) {
        ...
        try (Transaction tx = db.beginTx()) {
            ...
            if (indexingOn) {
                // put to index
                Index<Node> usernameIndex = db.index().forNodes(UserIndex.username.getIndexName(), UserIndex.username.getIndexType());
            ...
            LOGGER.log(Level.INFO,
                    String.format("UserController: TX: userId=%s, username=%s, processTime=%dms", userId, username, processTimeTx));
